...
async function refreshToken(req, res) {
    try {
        ...
        if (!user) {
            return res.status(401).json({ message: "User not found with the refresh token you provided!" });
        }

        if (user.status !== "active") {
            return res.status(401).json({ message: "Your Account is not active. Please verify your email first!" });
        }
